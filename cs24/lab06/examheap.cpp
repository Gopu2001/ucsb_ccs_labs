...
int main(int argc, char *argv[]) {
  ...
  if (t == 1) {
    ...
    Heap h;
    srand(time(0));
    int arand = rand() % 1000; // arand is a random integer between 0 and 999
    int brand = arand -10;
    h.push(arand);
    assert(h.top()==arand);
    h.pop();
  ...
  for (int i=0; i < numvals; i++) {
    int arand = rand() % 2000; // arand is a random integer from 0 to 1999
    data.push(arand);
    h.push(arand);
  }
  while(!data.empty()){
    assert(!h.empty());
    // cout << endl << data.top() << " " << h.top() << endl;
